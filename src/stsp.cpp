...
#define USEOLDLDRING 0				//whether to use the old uniformly spaced limb darkening ring system (0=new system)
...
//    printf("x[9]: %lf\n", x[8]);
    ...
    sprintf(seedfilename,"%s",filename);
    sprintf(rootname,"%s",filename);
	...
	//sprintf(filename,"%s","sample-l.in");
    //	sprintf(rootname,"%s",filename);
	for(j=0;j<64&&rootname[j]!=0;j++);
	if(rootname[j-3]=='.'&&rootname[j-2]=='i'&&rootname[j-1]=='n')
		rootname[j-3]=0;
...
#	if !QUIET
		printf("initializing with parameters from %s\n",filename);
#	endif
	j=initializestarplanetfileless(star,planet,filename,&lcstarttime,&lcfinishtime,&lcmaxlight,&ascale,&mcmcnpop,&mcmcmaxstepsortime,&partitionpop,&partitionsteps,readparam,&randomseed,seedfilename);
